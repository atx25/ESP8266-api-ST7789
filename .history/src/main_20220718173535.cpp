...
void wifi_start_connect()              //连接WIFI
{
  ...
  while (WiFi.status()!= WL_CONNECTED) //这个函数是wifi连接状态，返回wifi链接状态
        ...
        delay(200);
        tft.drawCentreString(".",30+i,140,0);
        i=i+10;
        if(i>=180){i=0;};
      }
...
void xianshi(){
  ...
  tft.drawCentreString(date[0],120,5+i,50);

  delay(250);

  String s=String(tianqiday[0]) +"转"+ String(tianqinight[0]);
  tft.drawCentreString(s.c_str(),120,5+i*2,0);

  delay(250);

  String ss1="气温:"+String(tempnight[0])+"-"+String(tempday[0]);
  tft.drawCentreString(ss1.c_str(),120,5+i*3,0);
  //明日天气
  tft.setTextColor(TFT_BLUE);         //设置文本颜色为lan色
  tft.drawCentreString(date[1],120,5+i*4,0);

  delay(250);

  String s1=(String(tianqiday[1]) +"转"+ String(tianqinight[1]));
  tft.drawCentreString(s1.c_str(),120,5+i*5,0);
  String ss2="气温:"+String(tempnight[1])+"-"+String(tempday[1]);
  tft.drawCentreString(ss2.c_str(),120,5+i*6,0);
  ...
  tft.drawCentreString(m,120,i*7+5,0);
  tft.unloadFont();
}
//解析程序,预报未来3天的天气
