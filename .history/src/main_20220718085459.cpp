...
//显示天气
void xianshi(String city[],String tianqiday[],String tianqinightp[],String date[]){
  pinMode(BLK, OUTPUT);
  tft.init();                           //初始化
  ...
  tft.setTextSize(4);
  tft.drawCentreString("1111",10,10,fontsimkai_16);
  tft.setTextColor(TFT_YELLOW);         //设置文本颜色为黄色
  tft.loadFont(fontsimkai_16);
  tft.setTextSize(4);
  tft.drawString(tianqi[1],10,20,fontsimkai_16);
  tft.unloadFont();
  analogWrite(BLK, 150);
}
//解析程序,预报未来3天的天气
...
//填充
 ...
 //明日天气
  ...
  tianqi = String (forecasts_0_casts_1_daytemp);
  xianshi(city,tianqiday,tianqinight,date);
