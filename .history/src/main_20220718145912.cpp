...
void xianshi(){
  ...
  tft.drawString(supdate,0)
  tft.setTextColor(TFT_YELLOW);         //设置文本颜色为黄色
  tft.drawString(date[0],0,215);
  ...
  const char* yubaoss1=ss1.c_str();
  tft.drawString(yubaoss1,0,95);
  //明日天气
  ...
  tft.drawString(yubao1,0,155);

  String ss2="气温:"+String(tempnight[0])+"-"+String(tempnight[0]);
  const char* yubaoss2=ss2.c_str();
  tft.drawString(yubaoss2,0,185);
  tft.unloadFont();
}
//解析程序,预报未来3天的天气
