...
void putjson(const char* content){
  ...
  city=String (forecasts_0_city);
  tianqi=String (forecasts_0_casts_0_dayweather);
  date=String (forecasts_0_casts_0_date);
...
void xianshi(String city,String tianqi,String date){
  pinMode(BLK, OUTPUT);
  tft.init();                           //初始化
  tft.fillScreen(TFT_WHITE);            //屏幕颜色
    tft.setCursor(30,i+20,3);
    tft.setTextColor(TFT_YELLOW);
    tft.setTextSize(3);
    tft.println(city);
    tft.setCursor(30,30+20,3);
    tft.setTextColor(TFT_YELLOW);
    tft.setTextSize(3);
    tft.println(tianqi);
    tft.setCursor(30,60+20,3);
    tft.setTextColor(TFT_YELLOW);
    tft.setTextSize(3);
    tft.println(date);
}
