...
void wifi_start_connect()              //连接WIFI
{
  ...
  while (WiFi.status()!= WL_CONNECTED) //这个函数是wifi连接状态，返回wifi链接状态
        ...
        tft.drawCentreString("content...",120,100,50);
        tft.drawCentreString(AP_SSID,120,150,50);
        tft.unloadFont();  
         delay(500);
         Serial.print(".");
        }
...
void xianshi(){
  ...
  String s=(String("白天:"tianqiday[0]) +" 夜间:"+ String(tianqinight[0]));
  ...
  String s1=(String("白天:"tianqiday[1]) +" 夜间:"+ String(tianqinight[1]));
